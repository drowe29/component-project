...
```generic
Top-Level container: Map<String, Sequence<Integer>>
- Map allows us associate an arbitrary number of games with their scores. Sequence<Integer> is the simplest variable‑length collection available in the OSU library that: 1) preserves insertion order (nice for reporting) 2) supports add, remove, and indexed access, and 3) already satisfies all Standard obligations.

Score element type: Integer
- Keeps the abstraction strictly about “scores”

Mutability: Mutable Map and mutable Sequences
- Matches the mutable nature of the component (scores can always be added or cleared).

Null Handling: never store null scores

Duplicate Scores: Allowed (a player can achieve the same score twice)
```
